...
export default function FinishedGoodsTab() {
  const [items, setItems] = useState<FinishedGoods[]>([]);
  ...
  function startEdit(item: FinishedGoods, field: string) {
    if (!editMode) return;
  ...
  async function saveEdit() {
    ...
    const res = await fetch(`/api/inventory/finished-goods/${editingCell.id}`, {
      method: "PUT",
  ...
  async function handleDelete(id: number) {
    ...
    const res = await fetch(`/api/inventory/finished-goods/${id}`, {
      method: "DELETE",
    });
    if (res.ok) {
      showToast("删除成功");
  ...
  const currentStock = (item: FinishedGoods) =>
      ...
      {creating && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30">
          <div className="w-full max-w-lg rounded-lg bg-white p-6 shadow-xl">
            <h3 className="mb-4 text-sm font-semibold">新增成品</h3>
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                <label className="text-xs text-gray-600">包装规格</label>
                <input
                  value={createForm.packagingSpec || ""}
                  onChange={(e) =>
                    setCreateForm({
                      ...createForm,
                      packagingSpec: e.target.value,
                    })
                  }
                  className="w-full rounded border border-gray-300 px-2 py-1 text-xs"
                />
              ...
              <div className="col-span-2">
                <label className="text-xs text-gray-600">库存类型</label>
                <select
                  value={createForm.stockType || "正常库存"}
                  onChange={(e) =>
                    setCreateForm({ ...createForm, stockType: e.target.value })
                  }
                  className="w-full rounded border border-gray-300 px-2 py-1 text-xs"
                >
                  <option>正常库存</option>
            ...
            <div className="space-y-3">
              <div>
                ...
                <select
                  ...
                  className="w-full rounded border border-gray-300 px-2 py-1 text-xs"
                >
                  <option value="inbound">生产入库</option>
                  <option value="outbound">销售发货</option>
