...
export default function RawMaterialTab() {
  ...
  const [createForm, setCreateForm] = useState<Partial<RawMaterial>>({});
  ...
  async function load() {
    ...
    const res = await fetch(
      `/api/inventory/raw-materials?${params.toString()}`,
    );
  ...
  async function saveEdit() {
    ...
    const res = await fetch(`/api/inventory/raw-materials/${editingCell.id}`, {
      method: "PUT",
  ...
  async function handleOperation() {
    ...
    if (res.ok) {
      showToast("操作成功");
      setShowOp(false);
      setOpForm({ opType: "purchase", targetId: 0, quantity: "", reason: "" });
      load();
    } else {
  ...
  const currentStock = (item: RawMaterial) =>
    item.lastBalance + item.currentPurchase - item.currentConsume;
      ...
      {creating && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30">
          <div className="w-full max-w-lg rounded-lg bg-white p-6 shadow-xl">
            <h3 className="mb-4 text-sm font-semibold">新增原辅料</h3>
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                <label className="text-xs text-gray-600">厂家</label>
                <input
            ...
            <div className="space-y-3">
              <div>
                ...
                >
                  ...
                  <option value="consume">生产领用</option>
