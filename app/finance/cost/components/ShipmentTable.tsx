    ...
    <div className="space-y-4">
      {summary && (
        <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
          <SummaryCard label="发货金额" value={fmt(summary.totalAmount as number)} />
          <SummaryCard label="已回款" value={fmt(summary.totalReceived as number)} />
          <SummaryCard label="未回款" value={fmt(summary.totalUnreceived as number)} />
          <SummaryCard
            label="回款率"
            value={`${(((summary.collectionRate as number) ?? 0) * 100).toFixed(1)}%`}
          />
        </div>
      )}
